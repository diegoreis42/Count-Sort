#include <stdlib.h>
#include <stdio.h>
#include <time.h>
#include <math.h>
#include "sorting.h"

int main()
{
  char *arq[] = {"../rand_files/100.txt", "../rand_files/1_000.txt", "../rand_files/10_000.txt", "../rand_files/100_000.txt", "../rand_files/1_000_000.txt"};
  char *vets[] = {"../ordered_vectors/insertion/100Ord.txt", "../ordered_vectors/insertion/1_000Ord.txt", "../ordered_vectors/insertion/10_000Ord.txt", "../ordered_vectors/insertion/100_000Ord.txt", "../ordered_vectors/insertion/1_000_000Ord.txt"};
  int Tamanho;
  unsigned int trc, cmp;

  trc = cmp = 0;
  FILE *f = fopen("../stats/insertion_stats.txt", "a+");
  if (f == NULL)
    return -1;

  printf("Insertion Sort\n-------------------\nSorting...");
  for (int i = 0; i < 5; i++)
  {
    int *v = read_file(arq[i], &Tamanho);
    insertion_sort(v, Tamanho, &trc, &cmp);
    print_file(vets[i], Tamanho, v);

    fprintf(f, "%d elements\n", (int)pow(10, (2 + i)));
    fprintf(f, "Swaps: %d\n", trc);
    fprintf(f, "Comparisions: %d\n", cmp);
    trc = cmp = 0;
  }

  return 0;
}
